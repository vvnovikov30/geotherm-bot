...
def test_idempotent_refresh_double_refresh_no_duplicates():
    ...
    try:
        ...
        assert stats1["items_enqueued"] > 0, (
            f"Первый refresh должен добавить элементы, " f"получили {stats1['items_enqueued']}"
        )
        ...
        assert stats2["items_enqueued"] == 0, (
            f"Второй refresh не должен добавлять элементы, " f"получили {stats2['items_enqueued']}"
        )

        # Проверяем, что items_deduped увеличился
        assert stats2["items_deduped"] > 0, (
            f"Второй refresh должен показывать дедупликацию, " f"получили {stats2['items_deduped']}"
        )

        # 7) Проверяем, что seen содержит записи
        assert content_queue.seen_exists(
            external_id_to_check
        ...
        assert stats3["items_enqueued"] == 0, (
            f"Третий refresh не должен добавлять элементы, " f"получили {stats3['items_enqueued']}"
        )

        print("[OK] Идемпотентность проверена:")
        print(f"  Первый refresh: {stats1['items_enqueued']} элементов")
        print(
            f"  Второй refresh: {stats2['items_enqueued']} элементов, "
            f"{stats2['items_deduped']} дедуплицировано"
        )
        print(
            f"  Третий refresh: {stats3['items_enqueued']} элементов, "
