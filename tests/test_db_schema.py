...
def test_db_schema():
    ...
    try:
        ...
        assert (
            "UNIQUE(chat_id, message_thread_id)" in topics_sql
        ), "UNIQUE constraint на topics должен быть"
        ...
        assert (
            "UNIQUE(topic_id, external_id)" in queue_sql
        ), "UNIQUE constraint на content_queue должен быть"
        print("[OK] UNIQUE constraints присутствуют")

        # Проверяем FOREIGN KEY constraint
        assert (
            "FOREIGN KEY(topic_id) REFERENCES topics(id) ON DELETE CASCADE" in queue_sql
        ), "FOREIGN KEY constraint должен быть"
