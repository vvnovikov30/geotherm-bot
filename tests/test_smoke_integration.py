...
def test_smoke_integration():
    ...
    try:
        ...
        assert count_after_dry == count_after_refresh, (
            f"В dry_run очередь не должна уменьшиться: "
            f"{count_after_dry} == {count_after_refresh}"
        )
        ...
        assert len(notifier.calls) == 1, (
            f"FakeNotifier должен быть вызван 1 раз, " f"получили {len(notifier.calls)}"
        )

        count_after_publish = content_queue.count_new(topic.id)
        assert count_after_publish == count_after_refresh - 1, (
            f"Очередь должна уменьшиться на 1: "
            f"{count_after_publish} == {count_after_refresh - 1}"
        )
        ...
        assert (
            topic_after_publish.last_post_at is not None
        ), "last_post_at должен обновиться после publish"
